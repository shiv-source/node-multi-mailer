...
/**
 * 
 * @param {Array.<string>} attachments - attachments are the list of selected files
 * @param {object} config  - multiMailer configuration
...
/**
 * 
 * @param {object} config - config is the multiMailer configuraion
 ...
 * @returns html
 */
Resolver.prototype.ejsTemplate = async function ( config, fileName , data) {

    let templatePath = await this.getFilePath( config._templateFolderPath  , fileName);
    let html = await await ejs.renderFile( templatePath , data );
